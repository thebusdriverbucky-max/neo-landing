...
export default function MessagesManager() {
  ...
  return (
    <section id="messages" className="p-8 bg-neutral-900 rounded-2xl border border-neutral-800 space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">Messages</h2>
        {bookings.length > 0 && (
          <button
            onClick={handleClearAll}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded-lg transition-colors border border-red-400/20"
          >
        ...
        <div className="space-y-6">
          <div className="grid gap-4">
            {bookings.map((booking) => (
              <div
                key={booking.id}
                className="p-6 bg-neutral-800/50 border border-neutral-700/50 rounded-xl hover:border-neutral-600 transition-all group relative"
              >
                <button
                  onClick={() => handleDelete(booking.id)}
                  className="absolute top-4 right-4 p-2 text-neutral-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity bg-neutral-900 rounded-lg border border-neutral-700"
                  title="Delete message"
                >
                  <Trash2 className="w-4 h-4" />
                </button>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-8 mb-6 pr-12">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-neutral-200 font-medium">{booking.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-neutral-200 font-medium">{booking.phone}</p>
                    </div>
                  </div>
                  ...
                  {booking.date && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-neutral-200 font-medium">{booking.date}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-neutral-400 text-sm">{new Date(booking.createdAt).toLocaleString()}</p>
                    </div>
                  </div>
                </div>

                <div className="bg-neutral-900/80 p-5 rounded-xl border border-neutral-700/50 relative overflow-hidden">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-neutral-300 whitespace-pre-wrap leading-relaxed">{booking.message}</p>
                    </div>
                  </div>
                </div>
              </div>
